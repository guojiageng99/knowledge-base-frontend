...
import { Button, Empty, Input, Layout, List, Select, Space, Typography, message } from 'antd';
...
export default function AIAssistantPage() {
  const { conversations, currentConversation, availableModels, selectedModel, isLoading, isStreaming, currentResponse,
    fetchConversations, fetchModels, createConversation, selectConversation, deleteConversation, sendMessage, setSelectedModel } = useAIStore();
  ...
  return <Layout className="ai-layout">
    ...
    <Content className="ai-content">
      <Header className="ai-header"><Space><RobotOutlined /><Typography.Title level={4}>AI知识库助手</Typography.Title></Space><Select value={selectedModel} options={availableModels.map((m) => ({ value: m.key, label: m.displayName }))} onChange={setSelectedModel} placeholder="选择模型" /></Header>
